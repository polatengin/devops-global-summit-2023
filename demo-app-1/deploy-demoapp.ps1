...
function Deploy-DemoApp {
  ...
  if ($Confirmed -eq "y") {
    ...
    Push-Location -Path .\AspNetCore.Docs\aspnetcore\mvc\views\dependency-injection/6.0sample/WebViewInject

    az webapp up --name "benchpress-web-${EnvironmentSuffix}" --resource-group "benchpress-rg-${EnvironmentSuffix}" --location "${Location}" --sku "S1"
